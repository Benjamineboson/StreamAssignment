...
public class StreamAssignment {
    ...
    // SHOULD I BE WRAPPING WITH "Optional.of()" ??????????????

    @Test
    public void task7(){
        Person expected = new Person(5436, "Tea", "Håkansson", LocalDate.parse("1968-01-25"), Gender.FEMALE);
        Optional<Person> optional = Optional.of(people.stream()
    ...
    public void task10(){
        ...
        Optional<String> optional = Optional.of(people.stream()
                .filter(person -> person.getPersonId() == personId)
                .map(person -> (person.getDateOfBirth().getDayOfWeek()+" "+person.getDateOfBirth()
                .format(DateTimeFormatter.ofLocalizedDate(FormatStyle.LONG)).toUpperCase()))
                .findFirst()
                .get());
    ...
    public void task11(){
        ...
        double averageAge = people.stream()
                .mapToDouble(person -> personToAge.applyAsInt(person))
                .average()
                .getAsDouble();
    ...
    public void task13(){
        ...
        Map<String, List<Person>> personMap = people.stream()
                .collect(Collectors.groupingBy(key -> key.getLastName()));
    ...
    @Test
    public void task14(){
        LocalDate[] _2020_dates = Stream.iterate(LocalDate.of(2020,01,01),
                localDate -> localDate.isBefore(LocalDate.of(2021,01,01)),
